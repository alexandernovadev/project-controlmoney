...
import {
  MONTH_NAMES,
  formatDateShort,
  getSubscriptionOptionsFromPeriod,
...
export default function HomeScreen() {
  ...
  const [paymentMethods, setPaymentMethods] = useState<
    Record<string, { label: string; type: "cash" | "digital" }>
  >({});
  ...
  const onRefresh = async () => {
    ...
    try {
      ...
      const methodMap: Record<
        string,
        { label: string; type: "cash" | "digital" }
      > = {};
      methods.forEach((m) => {
        methodMap[m.id] = { label: m.label, type: m.type };
      });
  ...
  useEffect(() => {
    ...
    const unsubIncomes = subscribeIncomeTransactions(
      user.uid,
      setIncomes,
      options,
    );
    const unsubExpenses = subscribeExpenseTransactions(
      user.uid,
      setExpenses,
      options,
    );
  ...
  useEffect(() => {
    ...
    getIncomePaymentMethods(user.uid).then((methods) => {
      const map: Record<string, { label: string; type: "cash" | "digital" }> =
        {};
      methods.forEach((m) => {
        map[m.id] = { label: m.label, type: m.type };
      });
  ...
  const balanceByMethod = calculateBalanceByMethod(
    incomes,
    expenses,
    paymentMethods,
  );
          ...
          <View style={styles.section}>
            ...
            <View style={[styles.cardsRow, { flexWrap: "wrap" }]}>
              {balanceByMethod.map((method) => (
                <Card
                  key={method.id}
                  style={[styles.miniCard, { minWidth: "45%" }]}
                >
                  <View style={styles.miniCardHeader}>
                    <MaterialIcons
                      ...
                      size={16}
                      color={
                        method.type === "cash" ? Colors.success : Colors.accent
                      }
                    />
                  ...
                  <Text
                    style={[
                      ...
                      method.balance < 0 && { color: Colors.error },
                    ]}
                  >
                    {method.balance < 0 ? "-" : ""}$
                    {formatAmountNumber(Math.abs(method.balance))}
                  </Text>
                </Card>
              ))}
            </View>
          ...
          <View style={{ gap: Spacing.sm }}>
            {mostExpensive && (
              <Card style={styles.insightCard}>
                <View style={styles.insightRow}>
                  ...
                  <Text style={styles.insightDescText} numberOfLines={1}>
                    {mostExpensive.description}
                  </Text>
                </View>
                <View style={styles.insightRow}>
                  <Text style={styles.insightLabelText}>Más caro</Text>
                  <Text style={styles.insightPriceText}>
                    ${formatAmountNumber(mostExpensive.amount)}
                  </Text>
                </View>
              </Card>
            )}
            ...
            {mostRepeated && (
              <Card style={styles.insightCard}>
                ...
                <View style={styles.insightRow}>
                  <Text style={styles.insightLabelText}>
                    {mostRepeated.count} veces
                  </Text>
                  <Text style={styles.insightPriceText}>
                    ${formatAmountNumber(mostRepeated.total)}
                  </Text>
                </View>
              </Card>
            )}
            ...
            {cheapest && (
              <Card style={styles.insightCard}>
                ...
                <View style={styles.insightRow}>
                  ...
                  <Text style={styles.insightPriceText}>
                    ${formatAmountNumber(cheapest.amount)}
                  </Text>
                </View>
              </Card>
            )}
